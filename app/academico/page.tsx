      ...
      <div className="max-w-3xl mx-auto">
          ...
          {STEPS.map((s, i) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                i === step ? "bg-[#D20001] text-white" : i < step ? "bg-[#D20001]/15 text-[#D20001]" : "bg-gray-100 text-gray-400"
              }`}
            >
              {i + 1}. {s}
            </button>
        ...
        {step === 1 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm">
            ...
            <button onClick={() => setSecoes(defaultSecoesForNivel(form.nivel))} className="text-xs text-[#D20001] font-semibold underline underline-offset-2 mb-6">
              ...
              <div key={grupo} className="mb-6">
                ...
                <div className="grid sm:grid-cols-2 gap-2">
                  {secoesPorGrupo[grupo].map(s => (
                    <label key={s.id} className={`flex items-center gap-2.5 p-3 rounded-xl border-2 cursor-pointer transition-all ${secoes.has(s.id) ? "border-[#D20001] bg-[#D20001]/5" : "border-gray-100 hover:border-gray-300"}`}>
                      <input type="checkbox" checked={secoes.has(s.id)} onChange={() => toggleSecao(s.id)} className="w-4 h-4 accent-[#D20001]" />
                      <span className="text-sm text-[#2A0001]">{s.label}</span>
                      {s.geradaPorIA && <Sparkles className="w-3 h-3 text-[#D20001] ml-auto shrink-0" />}
                    </label>
                  ))}
                </div>
              </div>
        ...
        {step === 2 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm space-y-6">
            <div>
              <p className="text-sm font-bold text-[#2A0001] mb-1">Mostrar logótipo na capa?</p>
              <p className="text-xs text-gray-400 mb-3">Logo da escola, universidade ou empresa, centrado automaticamente.</p>
              <div className="flex gap-2">
                <button onClick={() => setMostrarLogo(true)} className={`px-4 py-2 rounded-xl text-sm font-semibold border-2 transition-all ${mostrarLogo ? "border-[#D20001] bg-[#D20001]/10 text-[#D20001]" : "border-gray-200 text-gray-500"}`}>Sim</button>
                <button onClick={() => setMostrarLogo(false)} className={`px-4 py-2 rounded-xl text-sm font-semibold border-2 transition-all ${!mostrarLogo ? "border-[#D20001] bg-[#D20001]/10 text-[#D20001]" : "border-gray-200 text-gray-500"}`}>Não</button>
              </div>
            </div>
            ...
            <div className="bg-[#FFF8F8] rounded-xl p-4 text-xs text-gray-500 leading-relaxed">
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-6 md:p-8 shadow-sm space-y-5">
            <div className="flex items-center gap-3 mb-1">
              <div className="w-10 h-10 rounded-xl bg-[#D20001]/10 flex items-center justify-center">
                <FileText className="w-5 h-5 text-[#D20001]" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-[#2A0001]">Resumo do teu trabalho</h2>
                <p className="text-xs text-gray-400">Revê antes de gerar o documento</p>
              </div>
            </div>

            <div className="grid sm:grid-cols-2 gap-3 text-sm">
              <div><span className="text-gray-400">Tema:</span> <span className="font-semibold text-[#2A0001]">{form.tema}</span></div>
              <div><span className="text-gray-400">Nível:</span> <span className="font-semibold text-[#2A0001]">{NIVEIS.find(n => n.id === form.nivel)?.label}</span></div>
              {form.area && <div><span className="text-gray-400">Área:</span> <span className="font-semibold text-[#2A0001]">{form.area}</span></div>}
              {form.instituicao && <div><span className="text-gray-400">Instituição:</span> <span className="font-semibold text-[#2A0001]">{form.instituicao}</span></div>}
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {SECOES.filter(s => secoes.has(s.id)).map(s => (
                  <span key={s.id} className="badge bg-[#D20001]/10 text-[#D20001]">{s.label}</span>
                ))}
              </div>
            </div>
        ...
        {/* STEP 4 — Gerar */}
        {step === 4 && (
          <div className="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm flex flex-col items-center justify-center text-center gap-5 min-h-[280px]">
            {gerando ? (
              <>
                <Loader2 className="w-10 h-10 text-[#D20001] animate-spin" />
                <div>
                  <p className="font-bold text-[#2A0001]">A construir o teu documento...</p>
              ...
              <>
                <div className="w-16 h-16 bg-[#D20001]/10 rounded-2xl flex items-center justify-center">
                  <Sparkles className="w-8 h-8 text-[#D20001]" />
                </div>
                <div>
                  <p className="font-bold text-[#2A0001] text-lg">Pronto para gerar</p>
        ...
        {/* STEP 5 — Download */}
        {step === 5 && docBlob && (
          <div className="bg-white border border-gray-100 rounded-2xl p-8 shadow-sm flex flex-col items-center justify-center text-center gap-5">
            <div className="w-16 h-16 bg-[#D20001]/10 rounded-2xl flex items-center justify-center">
              <CheckCircle2 className="w-8 h-8 text-[#D20001]" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-[#2A0001] mb-1">Documento pronto!</h3>
