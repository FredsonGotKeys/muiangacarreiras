...
export async function POST(req: NextRequest) {
  ...
  const genero = str(cv.genero, 20) ?? "";
  const feminino = genero === "Feminino";
  const nascidoForma = feminino ? "nascida" : "nascido";
  const portadorForma = feminino ? "portadora" : "portador";
  const contactadoForma = feminino ? "contactada" : "contactado";
  const dataNascimento = str(cv.dataNascimento, 30) ?? "";
...
Coloco-me à inteira disposição de Vossa Excelência para prestar quaisquer esclarecimentos adicionais que considerem necessários, podendo ser ${contactadoForma} através do telefone ${telefone || "___________"} ou do email ${email || "___________"}.
