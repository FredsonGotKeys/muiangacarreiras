...
const values: { Icon: LucideIcon; iconBg: string; iconColor: string; title: string; desc: string }[] = [
  ...
  { Icon: Zap,    iconBg: "bg-green-50",  iconColor: "text-[#1D9E75]",  title: "Impacto",     desc: "Medimos o sucesso pelo impacto real que geramos nas pessoas, nas empresas e nas comunidades." },
...
export default function SobrePage() {
  return (
    <div className="bg-white min-h-screen">
      <section className="pt-28 sm:pt-32 pb-12 bg-[#0D0D0D] relative overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#1D9E75] opacity-10 rounded-full blur-[80px] -translate-x-1/3 translate-y-1/3 pointer-events-none" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-[#C9A84C]/20 text-[#C9A84C] mb-6">Quem somos</span>
      ...
      <section className="py-16 bg-gray-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0D0D0D] rounded-2xl p-8">
            <div className="w-8 h-1 bg-[#C9A84C] rounded-full mb-5" />
            <span className="badge bg-[#C9A84C]/20 text-[#C9A84C] mb-3 inline-flex">Visão</span>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-10">
            <span className="badge bg-[#C9A84C]/10 text-[#C9A84C] mb-3 inline-flex">Valores</span>
            <h2 className="text-3xl sm:text-4xl font-bold text-[#0D0D0D]">O que nos define</h2>
          </div>
            ...
            {values.map(({ Icon, iconBg, iconColor, title, desc }, i) => (
              <div key={title} className="service-card group">
                <div className="flex items-start justify-between">
                  ...
                  <span className="text-3xl font-bold text-gray-100 group-hover:text-[#C9A84C]/20 transition-colors">0{i + 1}</span>
                </div>
      ...
      <section className="py-16 bg-gray-50 border-t border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <span className="badge bg-[#C9A84C]/10 text-[#C9A84C] mb-3 inline-flex">Liderança</span>
          <h2 className="text-3xl sm:text-4xl font-bold text-[#0D0D0D] mb-12">Quem nos lidera</h2>
            ...
            <div className="pt-4 lg:pt-8">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {["Professor","Palestrante","Empresário","Pres. ADIEP","Lusofonia"].map((tag) => (
                  <span key={tag} className="badge bg-gray-100 text-gray-600 hover:bg-[#C9A84C] hover:text-white transition-all cursor-default">{tag}</span>
                ))}
              </div>
      ...
      <section className="bg-[#0D0D0D] py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="badge bg-[#C9A84C]/20 text-[#C9A84C] mb-6 inline-flex">Onde actuamos</span>
            ...
            {[{ code:"MZ", name:"Moçambique", desc:"Sede & mercado principal" },{ code:"AO", name:"Angola", desc:"PALOP & parcerias" },{ code:"BR", name:"Brasil", desc:"Lusofonia das Américas" },{ code:"PT", name:"Portugal", desc:"Rede lusófona europeia" }].map(({ code, name, desc }) => (
              <div key={code} className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center hover:bg-white/10 transition-colors">
                <div className="w-14 h-14 bg-[#C9A84C] rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <span className="text-sm font-bold text-[#0D0D0D]">{code}</span>
                </div>
